...
export const createUser = async (
  ...
  try {
    ...
    const existingIds = (existing.data.values || []).map((row) =>
      parseInt(row[0], 10)
    );
...
export const addProduct = async (
  ...
  const maxId =
    existingIds.length > 0
      ? Math.max(...existingIds.filter((id) => !isNaN(id)))
      : 0;
  const newId = maxId + 1;
...
export const likeProduct = async (
  ...
  const index = likes.findIndex(
    ([, uId, pId]) => parseInt(uId) == userId && parseInt(pId) == productId
  );

  if (index == -1) {
    // Auto-generate new ID
    const existingIds = likes
...
export const getLikedProductsByUser = async (
  userId: number
): Promise<any[]> => {
  const sheetsClient = await getSheetsClient();
  ...
  return products
    .map((row) => {
      ...
      if (parseInt(is_active) != 1 || !likedProductIds.has(productId))
        return null;
...
export const getCartByUser = async (userId: number): Promise<any[]> => {
  ...
  return products
    .map((row) => {
      ...
      if (parseInt(is_active) != 1 || !cartProductIds.has(productId))
        return null;
...
export const getAllProductsWithFlags = async (user: { id: number }) => {
  ...
  return products.map((product) => {
    ...
    return {
      ...product,
      is_product_liked: likedProductIds
        ? likedProductIds.has(productIdStr)
        : false,
      is_product_in_cart: cartProductIds
        ? cartProductIds.has(productIdStr)
        : false,
    };
  });
};

export const getUniqueProductCategoriesFromDB = async (): Promise<
...
> => {
  ...
  const products = allRows
    ...
    .filter((p: any) => p.is_active === "1" && p.category.trim());
  ...
  products.forEach((product: any) => {
    ...
    if (category && !categoryMap.has(category)) {
      categoryMap.set(category, product.image); // use the first image found for each category
    }
  });

  const uniqueCategoriesWithImages = Array.from(categoryMap.entries()).map(
